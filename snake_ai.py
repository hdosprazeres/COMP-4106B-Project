...
def deadEnd_paths():
        ...
        for row in range(rows):
            if (col, row) not in snake:
                ...
                if numberOfBarriers(node[0], snake) > 2:
                    deadEndPath = []
                    listOfDeadEndPaths.append(getDeadEndPath(node, deadEndPath, col, row))
    ...
    for path in listOfDeadEndPaths:
        for node in path:
           game_data["graph"][(node[0], node[1])][2] = game_data["graph"][(node[0], node[1])][2]*100 #temporary to test, need a proper method of assigning cost
